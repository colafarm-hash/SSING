# Figma Make — 온보딩 (O1 Login)

> 온보딩 = 로그인 1장. 가치 메시지 carousel·intro 슬라이드 없음. Toss/카카오T 스타일의 미니멀 진입.
...
| 점검 | 기대 |
|---|---|
| `/login` 라우트 작동 | URL 직접 입력 시 진입 |
| 카카오 노란 버튼 (#FEE500) | 검정 텍스트, 그라데이션 없음 |
| Apple 검정 버튼 | 흰 텍스트, 그라데이션 없음 |
| 카카오/Apple 탭 | `navigate("/")` (C1로) |
| 가치 메시지·환영 인사 부재 | 미니멀 진입 |
| 약관 안내 | 작은 회색 텍스트, 인라인 링크 |

---

## 통합 단계 (Figma Make 결과 받은 후)

1. zip 받기 → `Create app wireframe`에 덮어쓰기
2. `routes.tsx`에 `O1Login` import + `/login` 라우트 자동 추가됨 확인
3. (선택) C1 자동 redirect — 미인증 시 `/login` 강제 — 백엔드 통합 후
