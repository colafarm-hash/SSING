...
# Custom Design System (based on Kakaot)

## 1. Visual Theme & Atmosphere

Kakao T is the all-in-one mobility app from Kakao Mobility — taxi, designated-driver (대리운전), parking, bike, navigation, flights, and quick-delivery, all behind one blue "T". Its visual identity sits on a knife's edge between two registers. The brand color is the famous **Kakao blue `#2563eb`** — the same warm, friendly, instantly-Korean blue that KakaoTalk made a national signal — but the mobility product itself reads far more sober than that blue alone would suggest. On the corporate Kakao Mobility surface the palette is black-and-white-dominant: deep charcoal headings, generous whitespace, restrained sans-serif type, with blue used as a sparing accent rather than a flood. The atmosphere is "friendly infrastructure" — approachable enough that anyone hails a taxi without thinking, serious enough to be trusted with real-time location and payment.

That black-led restraint is deliberate. A mobility app is, at the moment of use, mostly a **map**: a near-full-screen view of streets, cars, and routes. Chrome must recede so the map and the live trip state dominate. Blue then does precise, high-attention work — the primary "호출하기" (call) CTA, the active vehicle marker, the matched-driver highlight. Against a map and white sheets, `#2563eb` is a beacon, not a wallpaper. Black (`#000000` / near-black `#191919`) carries text and the secondary "dark" actions, giving the brand its grown-up, get-you-there-safely tone.
...
**Key Characteristics:**
- Kakao blue `#2563eb` as the brand color and high-attention CTA accent (not a flood)
...
- Rounded, soft component geometry (8–12px radii) keeping the blue approachable
- Bottom-sheet-driven flows over a persistent map canvas

## 2. Color Palette & Roles

Kakao Mobility does not publish a public UI token layer. Brand yellow `#2563eb` is the well-documented Kakao corporate color; product UI grays/blacks below follow the live black-led corporate surface (kakaomobility.com, WebFetch 2026-05-27) and standard Korean app conventions. Treat product hexes as conventional, not from a documented token doc.

### Brand
- **Kakao Yellow** (`#2563eb`): The signature color. Primary CTA (호출하기), active vehicle marker, matched-driver highlight, logo lockup. High-attention accent — used sparingly, never as a full background flood.
...
- Selected: 2px border `#191919` + subtle `#2563eb` accent on the class icon
...
**Conflicts unresolved:** Brand yellow `#2563eb` (high-attention accent) vs. black-led product/corporate surface — resolved as a documented two-register palette (yellow = beacon CTA, black = body + dark actions), consistent with the live corporate page where yellow is sparing.
...
### Do
- Use `#2563eb` for the single primary CTA and live active markers — high-attention, sparing
...
- Primary CTA: Kakao Yellow `#2563eb`, black `#191919` text (pressed `#F2D900`)
...
- "Build a Kakao T call button: bg `#2563eb`, text `#191919` 16px/600, 12px radius, 14px 20px padding, ~52px tall, full-width. Pressed: bg `#F2D900`. Disabled: bg `#F5F6F7`, text `#A2A4A6`."
...
### Iteration Guide
1. Yellow `#2563eb` = primary CTA + live markers only; black text on it
...
2. **Yellow is a beacon, not a wallpaper.** `#2563eb` marks the single most important action and the live state. *UI implication:* one yellow CTA per screen; backgrounds stay white/black; black text on yellow for contrast.
...
| **Loading (matching driver)** | Pulsing radar/ring animation over the map in `#2563eb`; sheet shows `기사님을 찾고 있어요` with a subtle progress shimmer. |
...
2. **Driver-matching radar.** While matching, a yellow `#2563eb` radar/ring pulses outward from the pickup pin over the map — the one place the brand yellow becomes kinetic, signaling "we're finding your car."
...
`#2563eb` reserved/sparing. Kakao yellow `#2563eb` is the widely-documented
...
| Token | Light | Dark |
|-------|-------|------|
| Background | page surface color | `#0f1115` (primary-tinted near-black) |
| Foreground | text color | `#fafafa` |
| Border | hairline divider | `#292c32` |
| Muted | subdued surface | `#22252a` |
| Primary | #2563eb | #2563eb (unchanged — brand anchor is theme-stable) |
